...
size_pmask = size(source.p_mask);
elements = {};
for j = 1:size_pmask(2)
    for i = 1:size_pmask(1)
        if source.p_mask(i,j) > 0
            elements{end+1} = [i,j];
        end
    end
end
size_elements = size(elements);
t_offset = zeros(1,size_elements(2));

for i = 1:length(t_offset)
    len_i = dx*abs(det([(elements{1}-elements{end})',(elements{i}-elements{1})']))/norm((elements{1}-elements{end})');
    t_offset(i) = len_i/medium.sound_speed;
end
sourcep = zeros(length(t_offset), length(kgrid.t_array));
...
for i = 1:length(t_offset)
    sourcep(i,:) = source_mag * sin(2 * pi * source_freq * (kgrid.t_array+t_offset(i)));
end
source.p = sourcep;
%figure,plot(sourcep(2));
